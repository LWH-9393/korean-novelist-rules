# CLAUDE.md — 한국 소설 작법 21계명 진단 지침

이 파일은 Claude Code가 작업 디렉토리에서 자동으로 읽는 컨텍스트 파일입니다. 소설 작업 폴더에 이 파일을 복사해두면 그 디렉토리 안에서 작업할 때 Claude Code가 21계명 기반으로 원고를 진단·수정합니다.

원본 아이디어: [Threads @korgeek82](https://www.threads.com/@korgeek82?hl=ko)

> Claude Code의 Skills 시스템(자동 트리거)으로 쓰고 싶다면 같은 저장소의 `SKILL.md`를 `~/.claude/skills/korean-novelist-rules/`에 두면 됩니다. 이 `CLAUDE.md`는 특정 디렉토리 단위(소설 프로젝트 폴더)에서 자동 적용되길 원할 때 사용합니다.
